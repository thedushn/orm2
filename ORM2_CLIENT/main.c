...
void send_packet(pcap_t* p, struct packet* pkt, enum packet_type type, int data_size) {
    ...
    if (pcap_sendpacket(p, data, 1500) != 0)
    {
...
void client(struct makadd *c) {
   ...
   // printf("Sending file with size %d\n",(int) file_size);
    ...
    size = ftell(fp);
    file_size = size;
    size_t koliko_treba=file_size;
    fseek(fp,0,SEEK_SET);
        static int j=0;
    int file_pos=0;
    memset(pkt.data, 0, sizeof(pkt.data));
    memcpy(pkt.data,c->name,strlen(c->name));
    send_packet(c->p, &pkt, pkt_init,(int) strlen(c->name));
    if (wait_for_ack(c->p, ack_id)) {
        // packets[packet_to_send] = SENT;

        printf("Marking as sent\n");
    }
    memcpy(pkt.data,"fluffy.txg",strlen(c->name));
    send_packet(c->p, &pkt, pkt_init,(int) strlen(c->name));
    if (wait_for_ack(c->p, ack_id)) {
        // packets[packet_to_send] = SENT;
    ...
    while (1) {
        ...
        memset(pkt.data, 0, sizeof(pkt.data));
        size_t fsize = fread(pkt.data, sizeof(char), PACKET_SIZE, fp);

        pkt.header.offset=file_pos;
        file_pos+=(int)fsize;
        packet_to_send = -1;
        send_packet(c->p, &pkt, pkt_data,(int) fsize);
